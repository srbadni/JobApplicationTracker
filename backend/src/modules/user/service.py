...
class UserService:
    ...
    async def create(self, user: UserCreate, db: AsyncSession) -> dict[str, Any]:
        ...
        Raises:
            UserExistsError: If email or phone number already exists.
    ...
    async def update(self, user_id: int, user_update: UserUpdate, db: AsyncSession) -> dict[str, Any]:
        ...
        Raises:
            ...
            UserExistsError: If email or phone number conflicts with existing users.

        Note:
            Validates uniqueness when updating email or phone number.
    ...
    async def anonymize_user(self, user_id: int, db: AsyncSession) -> None:
        ...
        try:
            ...
            anonymize_data = UserAnonymize(
                name="[DELETED]",
                phone_number=f"098{user_id:08d}"[-11:],
